...
class Receiver extends AsyncEventEmitter {
	...
	constructor ({namespace, name, messageDecorator}, redis) {
		...
		this._receiverId = shortid.generate();
	...
	async commit (message, ...args) {
		...
		const defaultChannelName = `${this._namespace}:out:${message.method}:${message.tracker_id}`;
		...
		if (defaultChannelName !== commitOptions.channelName)
			commitOptions.channelName = this._namespace+':'+commitOptions.channelName;
