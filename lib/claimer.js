...
class Claimer extends AsyncEventEmitter {
	...
	async _filterPendingMessages (streamName) {
	...
	async _checkForTimedOutMessages (streamName) {
		...
		do {
			...
			[pendingMessages, lastMessage] = await this._filterPendingMessages(streamName);
	...
	run () {
		...
		this._interval = setInterval(async () => {
			try {
				...
				try {
					for (const streamName of this._streamNames)
						await this._checkForTimedOutMessages(streamName);
				} catch (error) {
